...
function resumenEjecutivo() {
  return [
    ...
    tabla([2700, 2100, 2100, 2126],
      ...
      [
        ['Total institucional', '1,45 %', '0,85 %', '41,0 %'],
        ['Sede', '1,78 %', '0,87 %', '51,4 %'],
        ['Carrera', '2,77 %', '2,26 %', '18,2 %'],
        ['Sede × carrera', '3,03 %', '2,44 %', '19,4 %'],
      ]),
    ...
    p('La historia reciente del propio estudiante pesa más que cualquier característica de su programa, y separa dos riesgos que no se parecen. El reiniciado —el que interrumpió uno o más semestres— se cae de la matrícula: continúa el 59,0 % de las veces frente al 86,0 % de un regular, y la brecha sigue abierta al final del plan. El recuperado —el que no dejó de matricularse pero repite el ciclo que perdió— se queda: continúa el 70,9 %. Lo suyo es el avance, no la permanencia.'),
    p([txt('A igualdad de ciclo, modalidad y semestre, los momios de continuar de un reiniciado son '), neg('la quinta parte'), txt(' de los de un regular (razón de momios 0,190) y los de un recuperado, más del doble de esos (0,424). La diferencia tampoco se agota en la retención: el recuperado avanza de ciclo sólo el 57,4 % de las veces frente al 86,4 % de un regular, repite el 20,9 % y vuelve a repetir el semestre siguiente el 18,2 %. Un modelo que promediara los tres grupos colocaría matrícula en ciclos que nadie va a cursar.')]),
    ...
    tabla([2700, 2100, 2100, 2126],
      ...
      [
        ['Total institucional', '1,44 %', '0,85 %', '40,8 %'],
        ['Sede', '1,65 %', '0,87 %', '47,4 %'],
        ['Carrera', '3,28 %', '2,26 %', '30,9 %'],
        ['Sede × carrera', '3,48 %', '2,44 %', '29,9 %'],
      ]),
    ...
    p([txt('El sesgo medio del total pasa además de +128 a −28 estudiantes: no sólo se reduce, cambia de signo y queda por debajo de tres décimas de punto sobre veinte mil. Conviene leer las dos ablaciones juntas: cada una mide lo que aporta '), cur('su'), txt(' dimensión dado que la otra ya está, y ambas salen grandes —del 18 % al 51 % la modalidad, del 30 % al 47 % la continuidad—. Eso quiere decir que '), neg('ninguna de las dos es redundante'), txt(': no están capturando la misma deriva por caminos distintos. La continuidad aporta más en los niveles de carrera y la modalidad en el de sede, que es coherente con lo que cada una describe.')]),
    ...
    tabla([3500, 1300, 1300, 1300, 1626],
      ...
      [
        ['Total institucional', '0,9 %', '0,7 %', '1,1 %', '1'],
        ['Sede', '0,9 %', '0,8 %', '1,1 %', '2'],
        ['Modalidad', '1,0 %', '1,1 %', '1,1 %', '3'],
        ['Continuidad', '3,7 %', '3,5 %', '1,7 %', '4'],
        ['Carrera', '2,0 %', '2,1 %', '3,2 %', '25'],
        ['Sede × carrera', '2,2 %', '2,2 %', '3,6 %', '42'],
        ['Sede × carrera × modalidad', '2,9 %', '3,1 %', '4,5 %', '83'],
        ['… × continuidad', '6,7 %', '7,0 %', '6,7 %', '225'],
        ['… × ciclo', '14,8 %', '16,6 %', '17,4 %', '812'],
        ['… × turno', '35,7 %', '45,3 %', '46,0 %', '1 151'],
      ]),
    ...
    tabla([3900, 1700, 1700, 1726],
      ...
      [
        ...
        ['+ continuidad de la matrícula', '0,0452', '±0,067', '1,00'],
      ]),
  ...
  return [
    ...
    tabla([1900, 1300, 1700, 1500, 1350, 1376],
      ...
      [
        ...
        ['Recuperado', '5 589', '70,9 %', '57,4 %', '20,9 %', '3,8 %'],
        ['Ingresante', '28 453', '61,7 %', '92,4 %', '5,2 %', '0,2 %'],
        ['Reiniciado', '2 694', '59,0 %', '68,4 %', '18,2 %', '5,5 %'],
      ]),
    ...
    ...vinetas([
      [neg('El reiniciado pierde matrícula. '), txt('Continúa el 59,0 % de las veces, veintisiete puntos por debajo de un regular y aún por debajo de un ingresante. Es un problema de retención puro.')],
      [neg('El recuperado pierde tiempo. '), txt('Continúa el 70,9 %, casi doce puntos por encima del reiniciado: no se cae de la universidad, porque de hecho nunca llegó a irse. Lo que hace es no avanzar: sólo el 57,4 % cambia de ciclo, frente al 86,4 % de un regular.')],
    ]),
    p([txt('La diferencia de retención no es un efecto de composición por ciclo, y el contraste separa a los dos grupos con holgura: controlando ciclo, modalidad y semestre en un modelo logit, los momios de continuar de un reiniciado son '), neg('la quinta parte'), txt(' de los de un regular (razón de momios 0,190), mientras que los de un recuperado son más de '), neg('el doble de esos'), txt(' (0,424). No son dos etiquetas de una misma población frágil: son dos poblaciones. El apartado 6.4 da el contraste completo.')]),
    ...
    p([txt('Las dos brechas se comportan además de forma distinta a lo largo del plan, y ése es el argumento para no fundirlas. La del '), neg('reiniciado apenas se estrecha'), txt(': veintinueve puntos en el ciclo 2 (55,2 % frente al 84,4 % de un regular) y todavía veintiuno en el ciclo 9 (73,4 % frente a 94,5 %). Es una cicatriz. La del '), neg('recuperado se cierra a la mitad'), txt(': de doce puntos en el ciclo 2 (72,8 % frente a 84,4 %) a menos de ocho en el ciclo 9 (86,8 % frente a 94,5 %). Quien repite un ciclo y lo supera vuelve a parecerse a cualquier otro estudiante; quien se ausentó un semestre, no del todo.')]),
    ...
    tabla([2226, 1360, 1360, 1360, 1360, 1360],
      ...
      [
        ...
        ['Reiniciado', '5,5 %', '18,2 %', '68,4 %', '5,2 %', '2,7 %'],
    ...
    p([txt('La condición tiene además '), neg('persistencia propia'), txt(': de los recuperados que continúan, el 18,2 % vuelve a repetir ciclo, frente al 7,6 % de los regulares. Repetir un ciclo multiplica por 2,4 la probabilidad de repetir el siguiente. No es un sorteo independiente cada semestre, y por eso la condición tiene que formar parte del estado y no ser una etiqueta calculada al final.')]),
    ...
    tabla([3000, 1500, 1500, 1500, 1526],
      ...
      [
        ['Continúa al semestre siguiente', '59,0 %', '69,5 %', '75,2 %', '85,3 %'],
