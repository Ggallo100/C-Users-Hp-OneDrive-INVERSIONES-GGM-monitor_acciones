...
function resumenEjecutivo() {
  return [
    ...
    p([txt('Los continuadores no son un bloque homogéneo, y el modelo no los trata como tal. Los separa en tres grupos según cómo llegan al semestre: '), neg('regulares'), txt(' —se matricularon también el semestre inmediato anterior—, '), neg('reiniciados'), txt(' —interrumpieron un semestre y volvieron— y '), neg('recuperados'), txt(' —interrumpieron dos o más—. Las tablas de resultados presentan esa apertura, y el apartado 1.3 explica por qué no es sólo una etiqueta.')]),
    ...
    p([txt('Quien vuelve tras una pausa vuelve frágil, y sigue siéndolo. A igualdad de ciclo y de modalidad, los momios de continuar de un '), neg('reiniciado'), txt(' son la quinta parte de los de un regular (razón de momios 0,21) y los de un '), neg('recuperado'), txt(' algo menos todavía (0,19). En el tercer ciclo, por ejemplo, continúa el 88,5 % de los regulares frente al 53,2 % de los reiniciados: treinta y cinco puntos de diferencia.')]),
    p('La diferencia no se agota en la retención. El reiniciado repite ciclo el 18,3 % de las veces frente al 9,3 % del regular, y avanza un ciclo sólo el 69,1 % frente al 84,7 %: vuelve a arrastrar los cursos que dejó. Un modelo que promediara ambos grupos colocaría a los que regresan en ciclos que no van a cursar.'),
    p([txt('Por eso la condición de llegada entra en el estado del modelo. Y entra sin coste de información: '), neg('no se declara en ningún sitio ni necesita matriz de transición'), txt('. La determina el rezago del propio flujo —quien se rematricula al semestre siguiente llega como regular, quien lo hace tras saltarse uno como reiniciado, tras saltarse dos o más como recuperado—, de modo que el desglose de las tablas sale de la recursión y suma exactamente el total.')]),
    tabla([2700, 2100, 2100, 2126],
      ['Nivel de agregación', 'Sin la dimensión', 'Con la dimensión', 'Reducción'],
      [
        ['Total institucional', '1,82 %', '1,43 %', '21,4 %'],
        ['Sede', '2,00 %', '1,43 %', '28,6 %'],
        ['Carrera', '3,53 %', '2,58 %', '26,8 %'],
        ['Sede × carrera', '3,72 %', '2,76 %', '25,7 %'],
      ]),
    ...
    p('El sesgo medio del total baja además de +202 a +121 estudiantes. Es la mejora más grande que ha aportado una sola dimensión a este modelo, por delante de la modalidad.'),
    ...
    tabla([3500, 1300, 1300, 1300, 1626],
      ...
      [
        ['Total institucional', '1,4 %', '0,9 %', '2,6 %', '1'],
        ['Sede', '1,4 %', '0,9 %', '2,6 %', '2'],
        ['Modalidad', '1,4 %', '1,0 %', '2,6 %', '3'],
        ['Continuidad', '1,6 %', '1,3 %', '2,8 %', '4'],
        ['Carrera', '2,3 %', '2,0 %', '4,5 %', '25'],
        ['Sede × carrera', '2,4 %', '2,2 %', '4,8 %', '42'],
        ['Sede × carrera × modalidad', '3,0 %', '3,1 %', '5,5 %', '83'],
        ['… × continuidad', '4,0 %', '4,4 %', '6,5 %', '225'],
        ['… × ciclo', '13,6 %', '14,9 %', '16,4 %', '812'],
        ['… × turno', '33,8 %', '43,2 %', '44,2 %', '1 151'],
      ]),
    ...
    tabla([3900, 1700, 1700, 1726],
      ...
      [
        ...
        ['+ continuidad de la matrícula', '0,0537', '±0,079', '0,90'],
      ]),
    ...
    p([txt('Casi la mitad de lo que el modelo original contabilizaba como choque aleatorio era '), neg('composición mal atribuida'), txt(': el peso creciente de poblaciones con menor retención. Una vez identificada, deja de ser incertidumbre y se convierte en estructura. Los escenarios resultantes son más estrechos porque el modelo sabe más, no porque se haya decidido estrecharlos.')]),
    ...
    nota('Una excepción que conviene conocer.', 'El nivel «Continuidad» —los cuatro totales institucionales de ingresantes, regulares, reiniciados y recuperados— es el único que queda por debajo del nominal, en torno al 68 %. Son cuatro celdas y doce comparaciones, de modo que el dato es ruidoso; pero apunta a algo real: el volumen de reingresos de un semestre concreto depende de decisiones institucionales —campañas de recuperación, amnistías de deuda— que el modelo no observa. Al planificar el acompañamiento a esa población conviene tomar el intervalo como un suelo, no como una garantía. El apartado 11.1 lo recoge.'),
...
function encargo() {
  return [
    ...
    ...vinetas([
      [neg('Regulares. '), txt('Se matricularon también el semestre inmediato anterior. Son el grueso: en torno al 80 % de la matrícula de un segundo semestre y al 62 % de la de un primero.')],
      [neg('Reiniciados. '), txt('Interrumpieron exactamente un semestre y volvieron. Entre el 1,9 % y el 2,6 % de la matrícula.')],
      [neg('Recuperados. '), txt('Interrumpieron dos o más semestres y volvieron. Entre el 0,6 % y el 1,5 %.')],
    ]),
    p('Los dos últimos grupos son pequeños en volumen y muy distintos en comportamiento, que es justo la combinación que hace que valga la pena separarlos: son poco visibles en el agregado y se comportan de forma que el promedio no describe.'),
    ...
    tabla([2200, 1100, 5726],
      ...
      [
        ...
        ['Continuidad de la matrícula', '4', 'Ingresante, y tres clases de continuador: regular, reiniciado y recuperado. No se declara ni se estima: la determina el rezago del flujo que trae al estudiante al semestre (apartado 5.5).'],
  ...
  return [
    ...
    tabla([1650, 922, 922, 922, 922, 922, 922, 922, 922],
      ...
      [
        ...
        ['Reiniciados', '—', '—', '347', '337', '362', '367', '560', '379'],
        ['Recuperados', '—', '—', '—', '86', '168', '156', '311', '216'],
      ], { tam: 17 }),
    ...
    nota('Un artefacto de la ventana, no un hecho.', 'Para ver un reiniciado hacen falta dos semestres previos y para ver un recuperado, tres. Por eso los reiniciados aparecen por primera vez en 2024-I y los recuperados en 2024-II: antes de esas fechas la categoría es inobservable, no inexistente. Las cifras de 2023 no deben leerse como que no hubiera reingresos, y el modelo tampoco las lee así — la estimación de cada tasa sólo usa los semestres en los que la categoría es observable.'),
    p([txt('Una vez la ventana lo permite, las proporciones son notablemente estables: los reiniciados oscilan entre el 1,9 % y el 2,6 % de la matrícula y los recuperados entre el 0,6 % y el 1,5 %. Juntos rondan el '), neg('3 % de la matrícula de cada semestre'), txt(', unos 600 estudiantes. Es poco volumen y mucha diferencia de comportamiento, que es exactamente el caso en que separar una población compensa.')]),
...
function hallazgos() {
  return [
    ...
    h2('4.3 Quien vuelve tras una pausa vuelve frágil, y sigue siéndolo'),
    ...
    tabla([2100, 1500, 1800, 1800, 1826],
      ['Condición de llegada', 'Observaciones', 'Continúa al semestre siguiente', 'Avanza un ciclo', 'Repite ciclo'],
      [
        ['Regular', '78 652', '84,9 %', '84,7 %', '9,3 %'],
        ['Ingresante', '28 453', '61,6 %', '92,4 %', '5,2 %'],
        ['Reiniciado', '1 973', '59,0 %', '69,1 %', '18,3 %'],
        ['Recuperado', '721', '55,9 %', '66,7 %', '18,4 %'],
      ]),
    pieTabla('Comportamiento según la continuidad de la matrícula. Valores crudos sobre los orígenes con destino observable; las observaciones son el recuento sin ponderar.'),
    p([txt('Un reiniciado continúa con una probabilidad parecida a la de un ingresante, veintiséis puntos por debajo de un regular. Y no es un efecto de composición por ciclo: controlando ciclo y modalidad en un modelo logit, los momios de continuar de un reiniciado son '), neg('la quinta parte'), txt(' de los de un regular (razón de momios 0,21) y los de un recuperado algo menos (0,19). El apartado 6.4 da el contraste completo.')]),
    ...
    p('La brecha se mantiene a lo largo de todo el plan, y ésa es la diferencia con la modalidad. Mientras las tres modalidades convergen a partir del cuarto ciclo, un reiniciado sigue por debajo de un regular en el ciclo 9 (77,7 % frente a 93,8 %). No es una barrera de entrada: es una cicatriz.'),
    p('El segundo efecto es sobre el avance. El reiniciado repite ciclo el 18,3 % de las veces, el doble que el regular, y avanza un ciclo sólo el 69,1 % frente al 84,7 %. Retrocede además con más frecuencia (5,6 % frente a 2,3 %). Vuelve a arrastrar los cursos que dejó, y eso desplaza su posición en el plan.'),

    h3('4.3.1 El efecto decae, pero no se cierra en un semestre'),
    p('Conviene medir cuánto dura. Tomando a los estudiantes que en algún momento interrumpieron, y contando los semestres transcurridos desde su regreso:'),
    tabla([3000, 1500, 1500, 1500, 1526],
      ...
      [
        ...
        ['Observaciones', '2 694', '1 081', '564', '69 675'],
      ]),
    pieTabla('Recuperación de la tasa de continuación tras un reingreso. La cicatriz se atenúa pero no ha desaparecido dos semestres después.'),
    nota('Decisión de diseño, y su límite.', 'La condición entra en el estado del modelo con memoria de un semestre: quien vuelve es reiniciado o recuperado en el semestre del regreso, y pasa a regular en el siguiente. Eso captura el grueso del efecto —el salto de 58 % a 85 % es el que mueve la aritmética— pero no la cola: el modelo asigna un 85 % a quien empíricamente está en el 69 %. Son 1 645 orígenes de unos 70 000 regulares, un 2,4 %, de modo que el sesgo que introduce en la tasa agregada de los regulares es de unas tres décimas de punto. Está medido y es pequeño, pero conviene conocerlo; el apartado 11.1 lo recoge como limitación.'),
