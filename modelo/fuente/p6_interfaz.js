...
/*
 * Ciclo máximo que un PROGRAMA puede ofrecer en el semestre T.
 *
 * Un programa que arranca no tiene todavía ciclos superiores: en su primer
 * semestre sólo existe el ciclo 1, en el segundo el 1 y el 2, y así. Es la
 * misma maduración que la de una sede nueva, y por el mismo motivo físico:
 * el plan se despliega a medida que la primera cohorte avanza. Sin esto, el
 * reparto estimado colocaría convalidaciones en ciclos que aún no se imparten.
 */
function topeCicloCarrera(ic, T) {
  const ap = S.aperturaPorIc[ic];
  if (!ap || !ap.enMaduracion) return 1e6;
  return Math.max(0, ap.cicloBase + (indicePeriodo(T) - indicePeriodo(ap.inicio)));
}
...
async function cargarArchivo(file) {
  try {
    ...
    /* Apertura de los PROGRAMAS nuevos. Un programa que no figura en el
       histórico empieza a impartirse en el primer semestre en que aparece, de
       modo que sólo ofrece el ciclo 1 —o el mayor que se declare en ese
       semestre, si se lanza convalidando desde un programa afín— y despliega
       el resto a razón de un ciclo por semestre. */
    S.aperturaPorIc = S.carreras.map(() => ({ enMaduracion: false }));
    S.carrerasNuevas.forEach(nom => {
      const ic = S.iC.get(nom);
      const suyas = filas.filter(r => r.ic === ic);
      const inicio = Math.min.apply(null, suyas.map(r => r.per));
      const decl = suyas.filter(r => r.per === inicio && r.ciclo !== null).map(r => r.ciclo);
      const cicloBase = decl.length ? Math.max.apply(null, decl) : 1;
      S.aperturaPorIc[ic] = { inicio, cicloBase, enMaduracion: true };
    });
    ...
    for (const r of filas) {
      ...
      const topeCar = topeCicloCarrera(r.ic, r.per);
      const tope = Math.min(topePlan, topeSed, topeCar);
      if (r.ciclo !== null && r.ciclo > tope) {
        if (topeCar <= topeSed && topeCar < topePlan) {
          recortes.plan++;
          if (recortes.plan <= 3) avisos.push({
            t: 'aviso', m: 'Fila ' + r.fila + ': ' + r.carrera + ' es un programa nuevo que ' +
              'en ' + rotuloPeriodo(r.per) + ' sólo imparte hasta el ciclo ' + tope +
              ' (inició en ' + rotuloPeriodo(S.aperturaPorIc[r.ic].inicio) +
              '); esos ' + fmtN(r.cant) + ' ingresantes se asignan al ciclo ' + tope + '.'
          });
        } else if (topeSed < topePlan) {
          recortes.sede++;
...
async function descargarPlantilla() {
  ...
  const inst = [
    ...
    ['Además despliega su plan ciclo a ciclo desde el semestre en que aparece: en el primero'],
    ['sólo imparte el ciclo 1, en el segundo el 1 y el 2, y así sucesivamente.'],
    ...
    ['          y 9 % al 5 o más.'],
    ['El reparto respeta el ciclo terminal del plan, el tope de maduración de una sede nueva y'],
    ['el de un programa nuevo, y conserva el total exacto: no se pierde ni se inventa a nadie.'],
    ['Un programa que no figura en el catálogo empieza a impartirse en el primer semestre en'],
    ['que aparece, de modo que ahí sólo existe el ciclo 1 y todos sus ingresantes van a él;'],
    ['despliega un ciclo más en cada semestre siguiente, conforme avanza su primera cohorte.'],
