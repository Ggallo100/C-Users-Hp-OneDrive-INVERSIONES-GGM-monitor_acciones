...
function validacion() {
  return [
    ...
    tabla([1600, 1650, 1250, 1300, 1500, 1726],
      ...
      [
        ['2025-I', '2025-II', '1', '16 353', '16 516', '+1,00 %'],
        ['2025-I', '2026-I', '2', '21 420', '21 220', '−0,93 %'],
        ['2025-I', '2026-II', '3', '20 330', '20 861', '+2,61 %'],
        ['2025-II', '2026-I', '1', '21 420', '21 104', '−1,48 %'],
        ['2025-II', '2026-II', '2', '20 330', '20 522', '+0,95 %'],
        ['2026-I', '2026-II', '1', '20 330', '20 680', '+1,72 %'],
      ]),
    pieTabla('Detalle del backtesting sobre el total institucional.'),
    p('El error a un semestre vista se mueve entre el 1,00 % y el 1,72 %, con una media del 1,40 %. Para una institución de veinte mil estudiantes, eso supone un error medio del orden de 280 estudiantes sobre el total.'),
    tabla([3000, 1900, 1900, 2226],
      ['Revisión del modelo', 'EPAP h = 1', 'EPAP medio', 'Sesgo medio'],
      [
        ['Ciclo, sede, carrera, turno', '2,16 %', '2,06 %', '+293'],
        ['+ modalidad de estudios', '1,73 %', '1,65 %', '+165'],
        ['+ continuidad de la matrícula', '1,40 %', '1,45 %', '+120'],
      ]),
    pieTabla('Evolución del error del total institucional a lo largo de las revisiones del modelo. Todas las cifras se miden sobre el mismo nivel de agregación —el total— y con la λ elegida por validación cruzada en cada especificación, de modo que son comparables.'),

    h2('8.4 El sesgo y la matrícula abierta de 2026-II'),
    p('El sesgo medio del total es de +120 estudiantes: el modelo proyecta algo por encima. Conviene explicar de dónde procede.'),
    p([txt('Las tres comparaciones que apuntan a 2026-II tienen sesgo positivo (+531, +192 y +350); las tres que apuntan a otros semestres suman un sesgo medio de '), neg('−117 estudiantes'), txt(', es decir, algo más de medio punto porcentual por debajo sobre una matrícula de más de veinte mil. Excluir el semestre sospechoso no sólo reduce el sesgo: le cambia el signo.')]),
    tabla([3400, 2700, 2926],
      ['Conjunto de comparaciones', 'Sesgo medio', 'Sobre la matrícula'],
      [
        ['Las seis comparaciones', '+120 estudiantes', '+0,59 %'],
        ['Sólo las que apuntan a 2026-II', '+358 estudiantes', '+1,76 %'],
        ['Excluyendo 2026-II', '−117 estudiantes', '−0,58 %'],
    ...
    p('Es relevante además que los dos contrastes de especificación apunten en la misma dirección. Incorporar la modalidad redujo el sesgo medio de +259 a +153 estudiantes, e incorporar la continuidad lo reduce de +202 a +121 sobre los mismos datos. Parte de lo que parecía sobreestimación sistemática era, en realidad, aplicar la retención media a dos poblaciones que retienen mucho menos: los estudiantes a distancia y los que vuelven de una interrupción.'),
...
function manual() {
  return [
    ...
    tabla([2600, 1400, 5026],
      ...
      [
        ['λ recencia (continuación)', '0,90', 'Ponderación de los semestres antiguos al estimar la continuación, el avance y el reparto por modalidad. Con 1,00 todo el histórico pesa igual.'],
...
function limitaciones() {
  return [
    ...
    ...numerada([
      ...
      [neg('El total de reiniciados y recuperados es la cifra menos fiable del modelo. '), txt('Su intervalo de predicción cubre el 67 % de los casos frente a un 80 % nominal, y no alcanza el nominal ni duplicando κ. El volumen de reingresos de un semestre depende de decisiones institucionales —campañas de recuperación, amnistías de deuda, flexibilización de requisitos— que no están en los datos. Conviene leer esa cifra como un orden de magnitud, y revisarla si hay una campaña en marcha.')],
      [neg('La memoria de la condición es de un semestre. '), txt('Quien vuelve de una pausa es reiniciado en el semestre del regreso y pasa a regular en el siguiente, aunque empíricamente siga por debajo de un regular durante dos o tres semestres más (apartado 4.3.1). El sesgo que eso introduce en la tasa de los regulares es de unas tres décimas de punto, medido sobre 1 645 orígenes de unos 70 000.')],
      ...
      [neg('Las categorías de continuidad están censuradas al inicio de la ventana. '), txt('Un reiniciado necesita dos semestres previos para ser observable y un recuperado, tres, de modo que 2023 no aporta ninguna observación de esas categorías y 2024 sólo parcialmente. El estimador lo controla —una matrícula entra en el conjunto de riesgo sólo si su condición es observable— pero la muestra efectiva de esos grupos es menor que la del resto, y ésa es una de las razones por las que λ se sitúa cerca de 1: descartar semestres antiguos los dejaría sin datos.')],
...
function anexos() {
  return [
    ...
    tabla([1150, 1700, 1550, 1550, 1550, 1526],
      ...
      [
        ['1', '28 870', '61,1 %', '1,9 %', '0,6 %', '0,4 %'],
        ['2', '15 622', '82,1 %', '2,9 %', '1,0 %', '0,4 %'],
        ['3', '13 403', '86,9 %', '3,2 %', '1,1 %', '0,4 %'],
        ['4', '10 613', '88,3 %', '3,0 %', '1,0 %', '0,4 %'],
        ['5', '9 418', '90,2 %', '3,3 %', '0,9 %', '0,5 %'],
        ['6', '7 149', '91,1 %', '3,0 %', '0,9 %', '0,3 %'],
        ['7', '7 132', '91,2 %', '3,2 %', '1,3 %', '0,5 %'],
        ['8', '5 579', '91,1 %', '2,8 %', '1,1 %', '0,4 %'],
        ['9', '5 220', '93,2 %', '2,5 %', '0,6 %', '0,4 %'],
        ['10', '4 708', '48,5 %', '2,0 %', '1,0 %', '0,2 %'],
        ['11', '2 085', '18,1 %', '1,9 %', '0,6 %', '0,5 %'],
      ]),
    pieTabla('Probabilidad de que la siguiente matrícula ocurra L semestres después, por ciclo de origen. Valores con la ponderación de recencia λ = 0,90 activa; las observaciones son el recuento sin ponderar. La caída de los ciclos 10 y 11 corresponde al egreso. Nótese que el reingreso tras pausa (L ≥ 2) es menos frecuente en el ciclo 1: quien abandona en el primer ciclo rara vez vuelve.'),
    p([txt('Las columnas L ≥ 2 de esta tabla son, por construcción, las que alimentan las condiciones de '), neg('reiniciado'), txt(' (L = 2) y '), neg('recuperado'), txt(' (L = 3 o 4) del semestre de destino. Sumadas dan el 3,9 % de las matrículas de origen, que es el orden de magnitud de esas dos poblaciones en la matrícula proyectada.')]),
    ...
    pieTabla('Continuación a un semestre por modalidad y ciclo. La brecha se concentra en los dos primeros ciclos y se cierra a partir del cuarto. Los guiones señalan ciclos sin evidencia propia de esa modalidad: la matrícula a distancia no pasa todavía del ciclo 7 y la semipresencial apenas llega al 11. En la proyección esas celdas se resuelven por contracción hacia el perfil del ciclo.'),
    ...
    tabla([1150, 1980, 1980, 1980, 1936],
      ['Ciclo', 'Regular', 'Reiniciado', 'Recuperado', 'Ingresante'],
      [
        ['1', '63,2 %  (790)', '49,6 %  (139)', '66,8 %  (81)', '61,0 %  (19 048)'],
        ['2', '83,4 %', '55,0 %', '53,5 %', '72,3 %'],
        ['3', '88,5 %  (8 788)', '52,8 %  (219)', '49,9 %  (88)', '67,1 %  (153)'],
        ['4', '89,7 %', '63,6 %', '51,6 %', '59,6 %'],
        ['5', '91,5 %', '57,2 %', '62,2 %', '68,4 %'],
        ['6', '92,3 %', '62,1 %', '57,7 %', '54,6 %'],
        ['7', '92,0 %', '72,4 %', '65,7 %', '61,1 %'],
        ['8', '92,0 %', '73,9 %', '53,9 %', '70,3 %'],
        ['9', '93,8 %  (3 531)', '77,7 %  (70)', '58,0 %  (30)', '—'],
        ['10', '48,8 %', '38,0 %', '35,2 %', '—'],
        ['11', '18,2 %  (1 352)', '15,7 %  (32)', '16,3 %  (10)', '—'],
      ], { tam: 17 }),
    ...
    p([txt('Tres lecturas conviene destacar. La primera es que el reiniciado del ciclo 3 (52,8 %) está treinta y seis puntos por debajo del regular del mismo ciclo (88,5 %): es la mayor diferencia que produce cualquier dimensión del modelo. La segunda es que la brecha se estrecha en los ciclos altos —en el 9, 77,7 % frente a 93,8 %— pero no llega a cerrarse. La tercera es que en los ciclos terminales las cuatro condiciones convergen: el egreso no distingue.')]),
    nota('Cuidado con las celdas pequeñas.', 'Los recuperados no pasan de un centenar de observaciones ponderadas en ningún ciclo, y los reiniciados de un par de centenares. Su cifra del ciclo 1 (66,8 %) queda incluso por encima de la del regular, lo que rompe la monotonía del resto de la tabla y es casi con seguridad ruido de muestreo. La contracción hacia el perfil del ciclo modera esos valores en la proyección —es justamente para lo que sirve— y el volumen afectado no mueve el agregado, pero la tabla se presenta en crudo para que el lector vea sobre qué evidencia descansa cada casilla.'),
    ...
    tabla([2600, 1600, 4826],
      ...
      [
        ['L = 1', '78,06 %', 'Se rematricula en el semestre inmediato siguiente. → Regular.'],
        ['L = 2', '2,68 %', 'Regresa tras interrumpir un semestre. → Reiniciado.'],
        ['L = 3', '0,87 %', 'Regresa tras interrumpir dos semestres. → Recuperado.'],
        ['L = 4', '0,39 %', 'Regresa tras interrumpir tres semestres. → Recuperado.'],
        ['No vuelve', '17,99 %', 'Deserción o egreso, según el ciclo.'],
      ], { al: [AlignmentType.LEFT, AlignmentType.CENTER, AlignmentType.LEFT] }),
    ...
    tabla([2600, 6426],
      ...
      [
        ...
        ['Continuidad de la matrícula', 'Dimensión del estado que distingue al continuador regular —se matriculó también el semestre anterior— del reiniciado —interrumpió uno— y del recuperado —interrumpió dos o más—. No se estima: la determina el rezago del flujo.'],
        ...
        ['Razón de momios', 'Cociente entre los momios de dos grupos, siendo los momios p/(1−p). Una razón de 0,21 significa que los momios del primer grupo son la quinta parte de los del segundo. Es la medida natural del efecto en un modelo logit.'],
        ['Rezago', 'Número de semestres entre una matrícula y la siguiente del mismo estudiante. Un rezago mayor que 1 es un reingreso tras pausa, y determina la condición de llegada del estudiante al semestre de destino.'],
