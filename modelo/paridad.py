...
# continuación: [is,ic,im,id,ci,pa,ip, n0..3, k0..3]
celda=acum(D['cont'],7,2*LAG_MAX,lambda f:f"{f[0]}|{f[1]}|{f[2]}|{f[3]}|{f[4]}|{f[5]}",LAM,6)
carr =acum(D['cont'],7,2*LAG_MAX,lambda f:f"{f[1]}|{f[2]}|{f[3]}|{f[4]}|{f[5]}",LAM,6)
moda =acum(D['cont'],7,2*LAG_MAX,lambda f:f"{f[2]}|{f[3]}|{f[4]}|{f[5]}",LAM,6)
cond =acum(D['cont'],7,2*LAG_MAX,lambda f:f"{f[3]}|{f[4]}|{f[5]}",LAM,6)
cipar=acum(D['cont'],7,2*LAG_MAX,lambda f:f"{f[4]}|{f[5]}",LAM,6)
cic  =acum(D['cont'],7,2*LAG_MAX,lambda f:f"{f[4]}",LAM,6)
glob=[0.0]*(2*LAG_MAX)
for f in D['cont']:
    w=LAM**(tmax-f[6])
    for i in range(2*LAG_MAX): glob[i]+=f[7+i]*w
ks={}
for nom,m in [('celda',celda),('carrera',carr),('moda',moda),('cond',cond),
              ('ciclopar',cipar),('ciclo',cic)]:
    ks[nom]=[k_betabinom([a[LAG_MAX+L] for a in m.values()],[a[L] for a in m.values()])*FK
             for L in range(LAG_MAX)]
# avance: [ic,im,id,ci,ip, d...]
avc =acum(D['av'],5,ND,lambda f:f"{f[0]}|{f[1]}|{f[2]}|{f[3]}",LAM,4)
avm =acum(D['av'],5,ND,lambda f:f"{f[1]}|{f[2]}|{f[3]}",LAM,4)
avd =acum(D['av'],5,ND,lambda f:f"{f[2]}|{f[3]}",LAM,4)
avci=acum(D['av'],5,ND,lambda f:f"{f[3]}",LAM,4)
avg=[0.0]*ND
for f in D['av']:
    w=LAM**(tmax-f[4])
    for i in range(ND): avg[i]+=f[5+i]*w
# turno: [is,im,id,ci,it,ip, dest...]
tuc=acum(D['tu'],6,NT,lambda f:f"{f[0]}|{f[1]}|{f[2]}|{f[3]}|{f[4]}",LAM,5)
tud=acum(D['tu'],6,NT,lambda f:f"{f[0]}|{f[1]}|{f[2]}|{f[4]}",LAM,5)
tum=acum(D['tu'],6,NT,lambda f:f"{f[0]}|{f[1]}|{f[4]}",LAM,5)
tus=acum(D['tu'],6,NT,lambda f:f"{f[0]}|{f[4]}",LAM,5)
...
par={'periodos':D['periodos'],'turnos':list(range(NT)),'sedes':D['sedes'],
 ...
 'cont_cond':nk(cond),'cont_ciclopar':nk(cipar),'cont_ciclo':nk(cic),
...
mod=Modelo(par)
stock0={}
for ip,is_,ic,im,id_,ci,it,v in D['stock']:
    stock0.setdefault(D['periodos'][ip],{})[(str(is_),str(ic),str(im),str(id_),ci,it)]=float(v)
obs={}
...
out={'k':{'celda':ks['celda'][0],'moda':ks['moda'][0],'cond':ks['cond'][0],
          'nuevos':par['k_nuevos'],
     ...
     'porCond':{str(T):{c:sum(v for k,v in cen[T].items() if k[3]==str(i))
                        for i,c in enumerate(D['condiciones'])} for T in fut}}
assert abs(sum(out['tot'][str(fut[0])].values())) > 0
