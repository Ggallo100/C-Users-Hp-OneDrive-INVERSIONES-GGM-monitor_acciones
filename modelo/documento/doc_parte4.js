  ...
  return [
    ...
    tabla([1600, 1650, 1250, 1300, 1500, 1726],
      ...
      [
        ...
        ['2025-I', '2026-II', '3', '20 527', '20 746', '+1,07 %'],
        ['2025-II', '2026-I', '1', '21 420', '21 099', '−1,50 %'],
        ['2025-II', '2026-II', '2', '20 527', '20 556', '+0,14 %'],
        ['2026-I', '2026-II', '1', '20 527', '20 657', '+0,63 %'],
      ]),
    pieTabla('Detalle del backtesting sobre el total institucional.'),
    p('El error a un semestre vista se mueve entre el 0,42 % y el 1,50 %, con una media del 0,85 %. Para una institución de veinte mil estudiantes, eso supone un error medio del orden de 175 estudiantes sobre el total.'),
    tabla([3600, 2400, 2226],
      ...
      [
        ...
        ['+ continuidad de la matrícula', '0,85 %', '−28'],
      ]),
    ...
    p('El sesgo medio del total es de −28 estudiantes, un −0,14 % de la matrícula: prácticamente nulo en el agregado. Conviene aun así descomponerlo, porque esa cifra pequeña es la suma de dos sesgos de signo contrario, y porque su evolución entre dos extracciones de la misma base permite contrastar una hipótesis.'),
    p([txt('Las tres comparaciones que apuntan a 2026-II siguen teniendo sesgo positivo; las tres que apuntan a otros semestres suman un sesgo medio de '), neg('−181 estudiantes'), txt(', es decir, algo menos de un punto porcentual por debajo sobre una matrícula de unos veinte mil.')]),
    tabla([3400, 2700, 2926],
      ['Conjunto de comparaciones', 'Sesgo medio', 'Sobre la matrícula'],
      [
        ['Las seis comparaciones', '−28 estudiantes', '−0,14 %'],
        ['Sólo las que apuntan a 2026-II', '+126 estudiantes', '+0,61 %'],
    ...
    p([txt('La versión anterior de este documento, escrita sobre una extracción de la misma base cuatro días más temprana, atribuía el sesgo a que la matrícula de 2026-II seguía abierta, y dejaba por escrito cómo comprobarlo: reestimar con un cierre más completo y ver si el sesgo de esas tres comparaciones se reducía. '), neg('La comprobación se ha hecho y el resultado es el esperado'), txt('.')]),
    tabla([2600, 2200, 2200, 2026],
      ['Extracción de la base', '10 de septiembre', '14 de septiembre', 'Variación'],
      [
        ['Matrículas de 2026-II', '20 331', '20 527', '+196'],
        ['Sesgo en las tres de 2026-II', '+245', '+126', '−49 %'],
        ['Sesgo medio de las seis', '+32', '−28', 'cambia de signo'],
        ['EPAP del total', '1,149 %', '0,854 %', '−25,7 %'],
      ], { al: [AlignmentType.LEFT, AlignmentType.CENTER, AlignmentType.CENTER, AlignmentType.CENTER] }),
    pieTabla('Efecto de completar 2026-II sobre el diagnóstico de sesgo. Las dos columnas son la misma base histórica extraída con cuatro días de diferencia; el resto de semestres es idéntico en ambas.'),
    p('Doscientas altas nuevas —un 1 % de ese semestre— bastaron para reducir a la mitad el sesgo de las comparaciones que lo tienen por objetivo y para bajar el error del total una cuarta parte. El modelo no estaba sobreestimando: estaba proyectando una matrícula completa contra un dato incompleto.'),
    p([txt('El sesgo residual de +126 que queda en 2026-II es coherente con lo que el apartado 3.4.2 estima que aún falta: unas doscientas altas, un 1 %. Si esa lectura es correcta, con el cierre definitivo ese sesgo debería acercarse a cero y el sesgo medio de las seis comparaciones quedar cerca de −180, que es el del resto de la serie. El procedimiento es reproducible: basta borrar '), cur('modelo/hist.pkl'), txt(' y reejecutar la canalización.')]),
    p('Es relevante además que los dos contrastes de especificación apunten en la misma dirección. Quitar la modalidad sube el sesgo medio de +32 a +201 estudiantes y quitar la continuidad lo sube de +32 a +187, sobre los mismos datos. Lo que parecía sobreestimación sistemática era, en buena parte, aplicar la retención media a poblaciones que retienen mucho menos: los estudiantes a distancia y los que vuelven de una interrupción.'),
...
function limitaciones() {
  return [
    ...
    ...numerada([
      ...
      [neg('El semestre 2026-II sigue admitiendo altas. '), txt('Está al 99 % de su campaña y faltan del orden de doscientas matrículas (apartado 3.4.2). Afecta al punto de partida de la recursión y deja un sesgo residual de +126 estudiantes en las comparaciones que apuntan a ese semestre. Conviene reestimar con el cierre definitivo, aunque el efecto que queda ya es pequeño.')],
...
function anexos() {
  return [
    ...
    tabla([1150, 1700, 1550, 1550, 1550, 1526],
      ...
      [
        ['1', '28 870', '61,5 %', '1,9 %', '0,6 %', '0,4 %'],
        ['2', '15 622', '82,4 %', '2,9 %', '1,0 %', '0,4 %'],
        ['3', '13 403', '87,2 %', '3,2 %', '1,0 %', '0,4 %'],
        ['4', '10 613', '88,5 %', '3,1 %', '1,0 %', '0,4 %'],
        ['5', '9 418', '90,5 %', '3,3 %', '0,9 %', '0,5 %'],
        ['6', '7 149', '91,4 %', '3,0 %', '0,9 %', '0,3 %'],
        ['7', '7 132', '91,3 %', '3,3 %', '1,3 %', '0,5 %'],
        ['8', '5 579', '91,4 %', '2,8 %', '1,1 %', '0,4 %'],
        ['9', '5 220', '93,4 %', '2,5 %', '0,6 %', '0,4 %'],
        ['10', '4 708', '49,3 %', '2,0 %', '1,0 %', '0,2 %'],
        ['11', '2 085', '19,0 %', '1,8 %', '0,7 %', '0,5 %'],
      ]),
    ...
    tabla([1150, 1980, 1980, 1980, 1936],
      ...
      [
        ['1', '71,6 %  (563)', '57,2 %  (596)', '56,6 %  (286)', '61,4 %  (27 425)'],
        ['2', '84,4 %  (13 670)', '72,8 %  (952)', '55,2 %  (422)', '72,5 %  (578)'],
        ['3', '89,5 %  (12 056)', '73,3 %  (719)', '52,8 %  (400)', '68,4 %  (228)'],
        ['4', '90,5 %  (9 460)', '80,7 %  (673)', '61,4 %  (350)', '59,2 %  (130)'],
        ['5', '91,9 %  (8 636)', '85,3 %  (442)', '60,7 %  (303)', '70,3 %  (37)'],
        ['6', '92,9 %  (6 597)', '82,5 %  (302)', '62,9 %  (224)', '57,7 %  (26)'],
        ['7', '92,4 %  (6 490)', '84,7 %  (398)', '72,6 %  (223)', '66,7 %  (21)'],
        ['8', '92,7 %  (4 969)', '85,9 %  (418)', '69,3 %  (189)', '—  (3)'],
        ['9', '94,5 %  (4 732)', '86,8 %  (357)', '73,4 %  (128)', '—  (3)'],
        ['10', '50,5 %  (4 103)', '41,4 %  (490)', '40,4 %  (114)', '—  (1)'],
        ['11', '20,3 %  (1 787)', '9,1 %  (242)', '20,0 %  (55)', '—  (1)'],
      ], { tam: 17 }),
    ...
    p([txt('Tres lecturas conviene destacar. La primera es que el reiniciado del ciclo 3 (52,8 %) está treinta y siete puntos por debajo del regular del mismo ciclo (89,5 %): es la mayor diferencia que produce cualquier dimensión del modelo. La segunda es que el recuperado ocupa sistemáticamente la posición intermedia, y que su brecha '), neg('se cierra'), txt(' —de doce puntos en el ciclo 2 a menos de ocho en el 9— mientras que la del reiniciado se mantiene por encima de los veinte. La tercera es que en los ciclos terminales las tres condiciones convergen: el egreso no distingue.')]),
    ...
    tabla([2600, 1600, 4826],
      ...
      [
        ['L = 1', '78,29 %', 'Se rematricula en el semestre inmediato siguiente. → Regular si cambia de ciclo, Recuperado si vuelve al mismo.'],
        ['L = 2', '2,69 %', 'Regresa tras interrumpir un semestre. → Reiniciado.'],
        ['L = 3', '0,87 %', 'Regresa tras interrumpir dos semestres. → Reiniciado.'],
        ['L = 4', '0,39 %', 'Regresa tras interrumpir tres semestres. → Reiniciado.'],
        ['No vuelve', '17,75 %', 'Deserción o egreso, según el ciclo.'],
      ], { al: [AlignmentType.LEFT, AlignmentType.CENTER, AlignmentType.LEFT] }),
    ...
    tabla([2600, 6426],
      ...
      [
        ...
        ['Razón de momios', 'Cociente entre los momios de dos grupos, siendo los momios p/(1−p). Una razón de 0,190 significa que los momios del primer grupo son algo menos de la quinta parte de los del segundo. Es la medida natural del efecto en un modelo logit.'],
